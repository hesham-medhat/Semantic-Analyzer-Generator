...
Parser ParserGenerator::generateParser(std::istream &rulesIstream,
  ...
  while (rulesIstream) {
    ...
    GrammarSymbol::ProductionPtr prod =
        getProduction(rulesIstream, terminals, nonTerminals, codeFrags);
    if (prod->empty()) {
    ...
    semanticAnalyzerGenerator.generateSemanticAnalyzer(productionId++, *symbol,
                                                       prod, codeFrags);
    while (skip(rulesIstream, "|")) {
      prod = getProduction(rulesIstream, terminals, nonTerminals, codeFrags);
      if (prod->empty()) {
      ...
      semanticAnalyzerGenerator.generateSemanticAnalyzer(
          productionId++, *symbol, prod, codeFrags);
    }
  }
  ...
  while (next != eof && !contains(delim, next)) {
    if (!codeBlock && next == '\\') {
      ...
      if (!contains(reserved, next)) {
        throw std::runtime_error(std::string("unrecognized escape sequence: ") +
                                 "\\" + next);
      }
    }
    input += is.get();
...
GrammarSymbol::ProductionPtr ParserGenerator::getProduction(
  ...
  while (!rhsTerm.empty()) {
    if (isValidSymbolName(rhsTerm)) {
      ...
      throw std::runtime_error(std::string("invalid symbol name in RHS: ") +
                               rhsTerm);
    }
...
void ParserGenerator::removeLeftRecursion(Parser &parser) {
  ...
  for (auto &nonTerminal : nonTerminals) {
    std::vector<GrammarSymbol::ProductionPtr> production =
        nonTerminal.second->productions;
    ...
    if (!a_dash_vector.empty()) {
      std::shared_ptr<NonTerminalSymbol> a_dash(
          new NonTerminalSymbol(nonTerminal.first + "'"));
    ...
    for (auto &nonTerminal : parser.nonTerminals) {
      ...
      for (int i = 0; i < production.size(); i++) {
        ...
        for (int j = i + 1; j < production.size(); j++) {
          if ((*production[i]->begin())->getName() ==
              (*production[j]->begin())->getName()) {
