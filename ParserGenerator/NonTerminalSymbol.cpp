...
NonTerminalSymbol::NonTerminalSymbol(std::string name)
      ...
      for (symIte = production->begin(); symIte != production->end();
        ...
        if (symbol->getType() == GrammarSymbol::Type::Terminal) {
          ...
          if (terminal->getName().compare("") != 0) {
            ...
            this->addTransition(epsilon, production);
          }
          break;
          ...
          for (callerIter = caller.begin(); callerIter != caller.end();
               callerIter++) {
            noRecursion =
                noRecursion && nonTerminal->getName().compare(*callerIter) != 0;
          }
          if (noRecursion) {
            std::unordered_set<TerminalSymbol::ptr> secFirst =
                nonTerminal->getFirst(caller);
            std::unordered_set<TerminalSymbol::ptr>::iterator secFirstIte;
            for (secFirstIte = secFirst.begin(); secFirstIte != secFirst.end();
                 secFirstIte++) {
              TerminalSymbol::ptr terminal =
                  std::dynamic_pointer_cast<TerminalSymbol>(*secFirstIte);
              if (terminal->getName().compare("") != 0) {
                first.insert(terminal);
                if (transitions.find(terminal) == transitions.end()) {
                  addTransition(terminal, production);
...
std::unordered_set<TerminalSymbol::ptr>
NonTerminalSymbol::getFollow(std::unordered_set<std::string> caller) {
  caller.insert(this->getName());
  if (!followCalculated) {
    ...
    for (pairIter = usingProductions.begin();
      ...
      GrammarSymbol::ProductionPtr production = (*pairIter).second;
        ...
        if ((*prodIter)->getName().compare(this->getName()) == 0) {
          ...
          while (prodIter2 != production->end()) {
            ...
            if (nextSymbol->getType() == GrammarSymbol::Type::Terminal) {
              TerminalSymbol::ptr terminal =
                  std::dynamic_pointer_cast<TerminalSymbol>(nextSymbol);
              ...
              NonTerminalSymbol::ptr nonTerminal =
                  std::dynamic_pointer_cast<NonTerminalSymbol>(nextSymbol);
              std::unordered_set<std::string> emptySet;
              std::unordered_set<TerminalSymbol::ptr> secFollow =
                  nonTerminal->getFirst(emptySet);

              std::unordered_set<TerminalSymbol::ptr>::iterator secFollowIter;
              for (secFollowIter = secFollow.begin();
                   secFollowIter != secFollow.end(); secFollowIter++) {
                if ((*secFollowIter)->getName().compare("") != 0) {
                  ...
                  for (const auto &p : nonTerminal->productions) {
                    ...
                    if (firstSymbol == *secFollowIter) {
                      nonTerminal->addTransition(epsilon, p);
                      break;
                      ...
                      if (fSymFirstSet.find(*secFollowIter) !=
                          fSymFirstSet.end()) {
                        nonTerminal->addTransition(epsilon, p);
                        break;
                      }
          ...
          for (callerIter = caller.begin(); callerIter != caller.end();
               callerIter++) {
            noRecursion =
                noRecursion && parent->getName().compare(*callerIter) != 0;
          }
          if ((prodIter2) == production->end() && noRecursion) {
            std::unordered_set<TerminalSymbol::ptr> secFollow =
                parent->getFollow(caller);
            std::unordered_set<TerminalSymbol::ptr>::iterator secFollowIter;
            for (secFollowIter = secFollow.begin();
                 secFollowIter != secFollow.end(); secFollowIter++) {
              follow.insert(*secFollowIter);
    ...
    GrammarSymbol::ProductionPtr epsProduction = std::make_shared<Production>();
    epsProduction->push_back(epsTerminal);
    GrammarSymbol::ProductionPtr synProduction = std::make_shared<Production>();
    synProduction->push_back(synTerminal);
    std::unordered_set<TerminalSymbol::ptr>::iterator followIter;
    for (followIter = follow.begin(); followIter != follow.end();
         followIter++) {
      if (transitions.find(*followIter) == transitions.end()) {
        if (hasEpsilonProduction) {
          addTransition(epsilon, epsProduction);
        } else {
          addTransition(*followIter, synProduction);
        }
...
void NonTerminalSymbol::addProduction(ProductionPtr production) {
  productions.push_back(production);
}

void NonTerminalSymbol::addUsingProduction(NonTerminalSymbol::ptr nonTerm,
...
NonTerminalSymbol::getProduction(TerminalSymbol::ptr terminal) {
...
void NonTerminalSymbol::saveProductions(
    std::ostream &out,
    std::unordered_map<std::string, std::shared_ptr<TerminalSymbol>> &terminals,
    std::unordered_map<std::shared_ptr<Production>, int> &productionIds) {
