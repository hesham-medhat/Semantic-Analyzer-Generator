...
Parser::Parser(
    ...
    std::unordered_map<std::string, std::shared_ptr<NonTerminalSymbol>>
        &nonTerminals)
    : lexicalAnalyzer(lexicalAnalyzer),
      startingSymbol(std::move(startingSymbol)), terminals(terminals),
...
Parser::Parser(LexicalAnalyzer &lexicalAnalyzer, std::istream &inputStream)
  ...
  for (int i = 0; i < nonterminalsCount; i++) {
    ...
    for (int terminalIndex = 0; getline(productionsSS, productionTok, ' ');
      ...
      if (productionTok == "$") {
        nonterminalsArray[i]->addTransition(terminalsArray[terminalIndex],
                                            synchProduction);
        ...
        std::shared_ptr<Sentence> newProduction = std::make_shared<Sentence>();
        ...
        while (getline(grammarSymbolsSS, grammarSymbolTok, '$')) {
          ...
          } else if (terminals.find(grammarSymbolTok) == terminals.end()) {
            if (nonTerminals.find(grammarSymbolTok) == nonTerminals.end()) {
...
void Parser::parseFullProgram(std::istream &) {
  ...
  typedef std::pair<std::shared_ptr<GrammarSymbol>,
                    std::shared_ptr<SemanticAnalyzer>>
      SymbolAnalyzerPair;
...
void Parser::save(std::ostream &out) {
  ...
  for (const auto &nonTerminalMapEntry : nonTerminals) {
    std::shared_ptr<NonTerminalSymbol> nonTerminal = nonTerminalMapEntry.second;
