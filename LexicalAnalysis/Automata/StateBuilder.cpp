...
std::shared_ptr<State> StateBuilder::buildState(std::string stateType,
                                                std::string tokenType,
                                                int priority) {
  Token token(tokenType, priority);
  ...
  if (stateType == "DFA") {
    ...
    throw std::runtime_error("StateBuilder got bad stateType specification: " +
                             stateType);
  }
