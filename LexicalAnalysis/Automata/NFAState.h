...
public:
  ...
  getNextState(char inputChar) override;
  ...
  viewTransitions() override;

  void explore(std::unordered_map<std::shared_ptr<State>, int> &collection,
               int *counter) override;

private:
  std::unordered_map<char, std::unordered_set<std::shared_ptr<State>>>
      transitions;
};
