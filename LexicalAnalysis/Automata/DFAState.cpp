...
DFAState::getNextState(char inputChar) {
  std::unordered_set<std::shared_ptr<State>> resultSet;
  auto search = transitions.find(inputChar);
  if (search != transitions.end())
    resultSet.insert(search->second);
  return resultSet;
}
...
std::vector<std::pair<char, std::unordered_set<std::shared_ptr<State>>>>
DFAState::viewTransitions() {
  std::vector<std::pair<char, std::unordered_set<std::shared_ptr<State>>>> view(
      transitions.size());
...
void DFAState::explore(
    std::unordered_map<std::shared_ptr<State>, int> &collection, int *counter) {
  for (auto transition : transitions) {
